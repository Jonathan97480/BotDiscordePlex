...
export async function createImagesTable(db: Database) {
...
export async function getLocalImagePath(db: Database, plexUrl: string): Promise<string | null> {
...
export async function insertImage(db: Database, plexUrl: string, localPath: string) {
...
export async function processPendingNotifications(db: Database, sendNotification: (userId: string, notification: any) => Promise<void>) {
...
export async function addUserToNotify(db: Database, serverId: string, userId: string) {
    await db.run('INSERT OR IGNORE INTO list_users_a_notifier (server_id, user_id) VALUES (?, ?)', serverId, userId);
}

export async function removeUserToNotify(db: Database, serverId: string, userId: string) {
    await db.run('DELETE FROM list_users_a_notifier WHERE server_id = ? AND user_id = ?', serverId, userId);
}

export async function shouldNotifyUser(db: Database, serverId: string, userId: string): Promise<boolean> {
...
export async function migratePendingNotificationsGuidAndIdMedia(db: Database) {
    ...
    for (const row of rows) {
        ...
        try {
            ...
            if (parsed && parsed.Metadata) {
                if (parsed.Metadata.guid) guid = parsed.Metadata.guid;
                if (parsed.Metadata.ratingKey) id_media = String(parsed.Metadata.ratingKey);
            }
        } catch (e) {
            // ignore
        }
...
export async function insertPendingNotification(db: Database, data: any) {
    ...
    if (data && data.Metadata) {
        if (data.Metadata.guid) guid = data.Metadata.guid;
        if (data.Metadata.ratingKey) id_media = String(data.Metadata.ratingKey);
    }
    // Vérifie la présence de l'id_media
    if (id_media) {
        const exists = await db.get('SELECT id FROM pending_notifications WHERE id_media = ?', id_media);
        if (exists) return; // Ne pas insérer de doublon
    }
    await db.run(
        'INSERT INTO pending_notifications (data, is_send, guid, id_media) VALUES (?, ?, ?, ?)',
        jsonData,
        0,
        guid,
        id_media
...
export async function showExists(db: Database, guid: string, id_media?: string): Promise<boolean> {
    let row;
    if (id_media) {
